...
const OrdersPage = () => {
  ...
  const mutationCompleteOrder = useMutation({
    mutationFn: (id: string) => updateOrderStatus(id, 4),
    onSuccess: () => {
  ...
  const columns: TableProps<OrderType>["columns"] = [
    {
      ...
      render: (_, record) => {
        ...
        const firstName =
          record.customer_id?.first_name || record.first_name || "N/A";
        const lastName =
          record.customer_id?.last_name || record.last_name || "";
        const email = record.customer_id?.email || record.email || "No email";
        const phone = record.customer_id?.phone || record.phone || "No phone";
